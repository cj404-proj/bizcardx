...
def storing_section(image):
    ...
    with st.form("biz-form"):
        ...
        submit = st.form_submit_button(label="Store")
        if submit:
            with st.spinner("Pushing data to the SQL."):
                time.sleep(5)
                new_data = [name,designation,mobile,email,url,company]
...
if selected == "Extract":
    ...
    st.write(pd.DataFrame(bizfuncs.fetch_data_from_sql(),columns=["name","designation","mobile","email","url","company"]))
